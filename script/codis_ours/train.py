...
from script.utils.cls_utils import make_optimizer, make_scheduler, compute_mae, compute_rmse, get_loader, gen_forget_schedule, js_loss_compute
...
def loss_soft_codis_plus(logits_1, logits_2, labels, temp, forget_rate, ind, noise_or_not, num_classes, device, co_lambda=0.1):
    soft_labels = hard2soft_label_batch(labels, num_classes).to(device)
    hard_labels = labels.to(device)

    loss_1 = F.cross_entropy(logits_1 / temp, hard_labels, reduction='none')- co_lambda * js_loss_compute(logits_1/ temp, logits_2/ temp,reduce=False)
    ind_1_sorted = np.argsort(loss_1.cpu().data).to(device)
    loss_1_sorted = loss_1[ind_1_sorted]

    loss_2 = F.cross_entropy(logits_2 / temp, hard_labels, reduction='none') - co_lambda * js_loss_compute(logits_1 / temp, logits_2/ temp, reduce=False)
...
def train(loader, epoch, net1, optimizer1, scheduler1, net2, optimizer2, scheduler2, temp, rate_schedule, noise_or_not, co_lambda, num_classes, device):
    net1.train()
    net2.train()
    ...
    for images, labels, _, indices in loader:
        ...
        num_remember, loss1, loss2, pure_ratio_1, pure_ratio_2 = \
              loss_soft_codis_plus(logits1, logits2, labels, temp, rate_schedule[epoch], ind, noise_or_not, num_classes, device, co_lambda)
...
def main(args):
    ...
    workdir = Path(args.workdir).joinpath(args.data_name, f'{args.noise_type}-{args.noise_level}', f'fold{args.fold}')
    ...
    log = f'{date}, {args.data_name}-{args.noise_type}-{args.noise_level}-fold{args.fold}'
    print(log)
    ...
    csv_root = Path('./dataset').joinpath(args.data_name, '5fold', f'{args.noise_type}-{args.noise_level}', f'fold{args.fold}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./script/codis_ours/config/codis_ours.yaml', help='(.yaml)')
    parser.add_argument('--workdir', type=str, default='./expr/codis_ours/')
