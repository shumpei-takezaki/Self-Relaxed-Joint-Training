...
from script.utils.cls_utils import make_optimizer, make_scheduler, compute_mae, compute_rmse, get_loader, gen_forget_schedule, kl_loss_compute
...
def loss_soft_jocor(logits_1, logits_2, labels, temp, forget_rate, ind, noise_or_not, num_classes, device, co_lambda=0.1):
    soft_labels = hard2soft_label_batch(labels, num_classes).to(device)
    hard_labels = labels.to(device)

    loss_pick_1 = F.cross_entropy(logits_1 / temp, hard_labels, reduction='none') * (1-co_lambda)
    loss_pick_2 = F.cross_entropy(logits_2 / temp, hard_labels, reduction='none') * (1-co_lambda)
    loss_pick = (loss_pick_1 + loss_pick_2 + co_lambda * kl_loss_compute(logits_1, logits_2,reduce=False) + co_lambda * kl_loss_compute(logits_2, logits_1, reduce=False)).cpu()
    ...
    loss_update_1 = softCrossEntropy(logits_1[ind_update], soft_labels[ind_update], reduction='none') * (1-co_lambda)
    loss_update_2 = softCrossEntropy(logits_2[ind_update], soft_labels[ind_update], reduction='none') * (1-co_lambda)
    loss_update = (loss_update_1 + loss_update_2 + co_lambda * kl_loss_compute(logits_1[ind_update], logits_2[ind_update],reduce=False) + co_lambda * kl_loss_compute(logits_2[ind_update], logits_1[ind_update], reduce=False)).cpu()
    
    return num_remember, torch.sum(loss_update)/num_remember, torch.sum(loss_update)/num_remember, pure_ratio, pure_ratio

def train(loader, epoch, net1, net2, optimizer, scheduler, temp, rate_schedule, noise_or_not, co_lambda, num_classes, device):
    net1.train()
    net2.train()
    ...
    for images, labels, _, indices in loader:
        ...
        num_remember, loss1, loss2, pure_ratio_1, pure_ratio_2 = \
              loss_soft_jocor(logits1, logits2, labels, temp, rate_schedule[epoch], ind, noise_or_not, num_classes, device, co_lambda)
...
def main(args):
    ...
    optimizer = make_optimizer(list(net1.parameters()) + list(net2.parameters()), **config.optimizer)
    ...
    for epoch in range(config.training.max_epoch):
        ...
        all_results['train'] = train(
                        ...
                        co_lambda=config.training.co_lambda,
    ...
    torch.save(optimizer.state_dict(), workdir.joinpath('ckpt', 'optim1_last.ckpt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./script/jocor_ours/config/jocor_ours.yaml', help='(.yaml)')
    parser.add_argument('--workdir', type=str, default='./expr/jocor_ours/')
