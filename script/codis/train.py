...
def loss_codis(logits1, logits2, labels, forget_rate, ind, noise_or_not, device, co_lambda=0.1):

    loss_1 = F.cross_entropy(logits1, labels, reduction='none') - co_lambda * js_loss_compute(logits1, logits2,reduce=False)
    ind_1_sorted = np.argsort(loss_1.cpu().data).to(device)
    loss_1_sorted = loss_1[ind_1_sorted]

    loss_2 = F.cross_entropy(logits2, labels, reduction='none') - co_lambda * js_loss_compute(logits1, logits2,reduce=False)
...
def train(loader, epoch, net1, optimizer1, scheduler1, net2, optimizer2, scheduler2, rate_schedule, noise_or_not, co_lambda, device):
    net1.train()
    net2.train()
    ...
    for images, labels, _, indices in loader:
        ...
        num_remember, loss1, loss2, pure_ratio_1, pure_ratio_2 = \
              loss_codis(logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, device, co_lambda)
