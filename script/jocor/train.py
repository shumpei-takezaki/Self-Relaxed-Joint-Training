...
def loss_jocor(logits1, logits2, labels, forget_rate, ind, noise_or_not, device, co_lambda=0.1):

    loss_pick_1 = F.cross_entropy(logits1, labels, reduction='none') * (1-co_lambda)
    loss_pick_2 = F.cross_entropy(logits2, labels, reduction='none') * (1-co_lambda)
    loss_pick = (loss_pick_1 + loss_pick_2 + co_lambda * kl_loss_compute(logits1, logits2,reduce=False) + co_lambda * kl_loss_compute(logits2, logits1, reduce=False)).cpu()
    ...
    loss = loss_pick[ind_update]
    
    return num_remember, torch.sum(loss)/num_remember, torch.sum(loss)/num_remember, pure_ratio, pure_ratio


def train(loader, epoch, net1, net2, optimizer, scheduler, rate_schedule, noise_or_not, co_lambda, device):
    net1.train()
    net2.train()
    ...
    for images, labels, _, indices in loader:
        ...
        num_remember, loss1, loss2, pure_ratio_1, pure_ratio_2 = \
              loss_jocor(logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, device, co_lambda)
