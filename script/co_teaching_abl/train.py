...
def loss_soft_coteaching(logits_1, logits_2, soft_labels, forget_rate, ind, noise_or_not, device):

    loss_1 = softCrossEntropy(logits_1, soft_labels, reduction='none')
    ind_1_sorted = np.argsort(loss_1.cpu().data).to(device)
    loss_1_sorted = loss_1[ind_1_sorted]

    loss_2 = softCrossEntropy(logits_2, soft_labels, reduction='none')
...
def train(loader, epoch, net1, optimizer1, scheduler1, net2, optimizer2, scheduler2, rate_schedule, noise_or_not, device):
    ...
    for images, labels, _, indices in loader:
        ...
        soft_labels = hard2soft_label_batch(labels).to(device)

        logits1 = net1(images)
        logits2 = net2(images)

        num_remember, loss1, loss2, pure_ratio_1, pure_ratio_2 = \
              loss_soft_coteaching(logits1, logits2, soft_labels, rate_schedule[epoch], ind, noise_or_not, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./script/co_teaching_abl/config/co_teaching_abl.yaml', help='(.yaml)')
    parser.add_argument('--workdir', type=str, default='./expr/co_teaching_abl/')
